...
```shell
docker run -it   --gpus 'all'   -v "${PWD}:/workspace"    -v "/home/student/minzod_mmdet3d:/workspace/minzod_mmdet3d" --name "yolov2-container" --env="DISPLAY" --env="QT_X11_NO_MITSHM=1" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw"  yolov2-image
```
or
```shell
